...
int des_encrypt(char * ming, char *key, char * out, unsigned char len)
{
	...
	if (len > 8)
	{
		return (-1);//can not supply this format
	}
	/*------------*/
	char m_t[8];
	/*------------*/
...
int des_decrypt(char * mi, char *key,  char * out, unsigned char len)
{
	...
	/* do */
	DES_Dfun(input, key, out);
	/* return len */
	return len;
}
/* advandage apps */
static char r_buffer[64 * 1024 * 1024];//up to 64M
/* encrypt a file */
int dencrypt_file(const char *path, char * key , unsigned int mode)
{
	...
	/* ok */
	size_t len = fread(r_buffer, 1, sizeof(r_buffer), fp);
	...
	if (mode == 1)
	{
		...
		memcpy(id_d, r_buffer, 8);
		/* */
		if (strcmp(id_d, "<olo><1>") != 0)
		{
			printf("file disencryped break\r\n");
			return (-2);
		}
		/*-real len-*/
		real_len = *((unsigned int *)&r_buffer[8]);
	}

	/*-----------*/
	if (len == sizeof(r_buffer))
	{
		printf("file size 64MB . Skip it : %s\r\n",path);
		return (-1);
	}
	/* overrade the file */
	fclose(fp);
	...
	while (1)
	{
		if (mode == 0)
		{
			des_encrypt(&r_buffer[pos], key, out, 8);
			/* copy data */
			memcpy(&r_buffer[pos], out, 8);
		}
		else if (mode == 1)
		{
			des_decrypt(&r_buffer[pos+12], key, out, 8);
			/* copy data */
			memcpy(&r_buffer[pos+12], out, 8);
		}
		/* incret */
		pos += 8;
		/* end ? */
		if (pos >= len)
		{
			if (mode == 0)
			{
				fwrite("<olo><1>", 1, 8, fp);
				fwrite(&len, 1, 4, fp);
				fwrite(r_buffer, 1, pos, fp);
			}
			else if (mode == 1)
			{
				fwrite(&r_buffer[12], 1, real_len, fp);
			}
			fclose(fp);
			break;
		}
